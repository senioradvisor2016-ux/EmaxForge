        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            HStack {
                Button("Choose Folder…") {
                    ...
                    // Fix: Set minimum size to prevent collapsed panel
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(spacing: 12) {
                ...
                if useImageIndex && imageCount > 1 {
                    reviewRow(icon: "square.stack", label: "Disks", value: "\(imageCount) images")
                    reviewRow(icon: "internaldrive", label: "HD10 (SCSI 1)", value: "\(sizeMB) MB Boot + OS")
                    reviewRow(icon: "internaldrive", label: "HD20+ (SCSI 2+)", value: "962 MB Data (each)")
                    reviewRow(icon: "music.note.list", label: "Banks", value: "\(bankFiles.count) total (distributed)")
                } else {
    ...
    private var doneStep: some View {
        VStack(spacing: 20) {
            ...
            if let url = createdImageURL {
                if imageCount > 1 {
                    VStack(spacing: 6) {
                        Text("HD10.hda = \(sizeMB) MB Boot (SCSI ID 1)")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Text("HD20.hda+ = 962 MB Data (SCSI ID 2+)")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                } else {
            ...
            do {
                ...
                for i in 0..<count {
                    ...
                    if i == 0 && includeOS && (osFileURL != nil || osDetected) {
                        ...
                        // ALWAYS use 962 MB for data disks (max capacity, cluster size 32KB)
                        let dataDiskSize = 962  // 1 GB minus overhead
                        try ImageCreator.createBlankImage(at: destURL, sizeMB: dataDiskSize)
                    }
                        ...
                        for (bi, bankURL) in banksForThisDisk {
                            ...
                            if ext == "eb2" {
                                do {
                                    ...
                                    print("⚠️ Failed to import bank \(bankURL.lastPathComponent): \(error.localizedDescription)")
                                    // Continue with other banks instead of failing completely
                                }
                            } else if FormatConverter.emuExtensions.contains(ext) {
                                let (imported, errors) = FormatConverter.convertAndImport(urls: [bankURL], into: destURL)
                                totalBankCount += imported.count
                                if !errors.isEmpty {
                                    print("⚠️ Errors importing \(bankURL.lastPathComponent): \(errors.map { $0.1.localizedDescription }.joined(separator: ", "))")
                                }
                            }
                        }
                    }
                ...
                if generateConfig {
                    ...
                    for i in 0..<count {
                        let currentScsiID = count > 1 ? (i + 1) : scsiID  // match creation: first=SCSI1, second=SCSI2
